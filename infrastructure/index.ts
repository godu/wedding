...
const USEast1Provider = new aws.Provider('awsProvider', {region: aws.Region.USEast1});
...
const githubOidcProviderArn = accountId.then(async accountId => awsnative.iam.getOidcProvider({
...
const weddingCertification = new aws.acm.Certificate('WeddingCertificate', {
...
const weddingCertificationValidationDomain = new aws.route53.Record('WeddingCertificateValidationDomain', {
	...
	zoneId: aws.route53.getZone({name: 'weberian.fr'}, {async: true}).then(zone => zone.zoneId),
	records: [weddingCertification.domainValidationOptions[0].resourceRecordValue],
	ttl: 60,
}, {provider: USEast1Provider});

new aws.acm.CertificateValidation('weddingValidation', {
...
const weddingDistributionHostedZoneId = pulumi.all([weddingDistribution.id])
	.apply(async ([id]) => aws.cloudfront.getDistribution({id})
...
const weddingHostedZoneId = aws.route53.getZone({name: 'weberian.fr'}).then(({zoneId}) => zoneId || '');
